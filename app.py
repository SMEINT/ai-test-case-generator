...
def fetch_jira_ticket_summary(ticket_id):
    ...
    if response.status_code == 200:
        ...
        return f"❌ Error fetching ticket summary: {response.status_code} - {response.text}", "N/A"
...
if selected_ticket:
    ticket_summary, ticket_priority = fetch_jira_ticket_summary(selected_ticket)

    st.markdown(f"**📝 Ticket Summary:** {ticket_summary}")
    st.markdown(f"**📌 Priority:** {ticket_priority}")
